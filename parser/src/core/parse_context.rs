...
impl <'a>ParseContext<'a> {
    ...
    pub fn add_error(&mut self, context: &'a ParseContext<'a>) -> Self {
        if self.parse_error_opt.is_some() && context.parse_error_opt.is_some() {
            let parse_error = context.parse_error_opt.as_ref().unwrap();
            if self.label.eq("") || self.label.eq(parse_error.label) {
                ParseContext {
                    label: self.label,
                    location: std::cmp::max(self.location, parse_error.location),
                    parse_error_opt: self.parse_error_opt.as_ref().map(move |e| e.clone().add_error(parse_error))
                }
            } else {
                ParseContext {
                    label: self.label,
                    location: self.location,
                    parse_error_opt: self.parse_error_opt.as_ref().map(|e| e.clone().location_change(std::cmp::max(self.location, parse_error.location)))
                }    
            }
        } else {
