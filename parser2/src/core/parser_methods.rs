...
impl<'a, A> ParserMethods<'a> for Parser<'a, A> {
        ...
        B: Clone + 'a,
    {
        ...
        Parser::new(move |context, input, location| {
            match self.parse(context, input, location) {
                (context, ParseResult::Success { value, location }) => {
                    let value_a = value;
                    match parser2.parse(context, input, location) {
                        (context, ParseResult::Success { value, location }) => {
                            let value_b = value;
                            (context, ParseResult::successful((value_a, value_b), location))
                        },
